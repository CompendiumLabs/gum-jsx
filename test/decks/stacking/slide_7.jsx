// Stacking scenarios / 07. Read README.md for the deck and render commands.
...
return <Slide background={paper} border={0} margin={0} padding={0}
  ...
  <Group coord={[0, 0, 48, 27]} aspect={16 / 9}>
    <Label x={2} y={1.25} width={44} size={0.55} color={teal} family={mono}>
      STACKING / 07 OF 07 / UNIFIED METRICS
    </Label>
    <Label x={2} y={2.55} width={44} size={1.4} weight={bold}>A column shares width.</Label>
    <Label x={2} y={4.55} width={44} size={0.76} color={muted}>Turn the stacking direction: width becomes the common cross dimension.</Label>
    <Label x={2} y={6} width={44} size={0.5} color={muted} family={mono}>
      Dashed outlines = allocated boxes. Numbers = width × height.
    </Label>
    <Panel x={2} title="Unsized figure follows width" stack={first} unit={0.72}
    ...
    <Panel x={24.5} title="Explicit height stays fixed" stack={second} unit={0.72}
    ...
    <Label x={2} y={25.3} width={44} size={0.57} color={muted}>{"Rows share height; columns offer width. Measured children retain their content scale."}</Label>
  </Group>
</Slide>
