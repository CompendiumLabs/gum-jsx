// Stacking scenarios / 05. Read README.md for the deck and render commands.
...
return <Slide background={paper} border={0} margin={0} padding={0}
  ...
  <Group coord={[0, 0, 48, 27]} aspect={16 / 9}>
    <Label x={2} y={1.25} width={44} size={0.55} color={teal} family={mono}>
      STACKING / 05 OF 07 / UNIFIED METRICS
    </Label>
    <Label x={2} y={2.55} width={44} size={1.4} weight={bold}>Text wraps when the width changes.</Label>
    <Label x={2} y={4.55} width={44} size={0.76} color={muted}>A fixed 2 × 1 figure keeps its size while the paragraph uses the remaining width.</Label>
    <Label x={2} y={6} width={44} size={0.5} color={muted} family={mono}>
      Dashed outlines = allocated boxes. Numbers = width × height.
    </Label>
    <Panel x={2} title="Row width 12: text gets 9" stack={first} unit={1.1}
    ...
    <Panel x={24.5} title="Row width 8: text gets 5" stack={second} unit={1.1}
    ...
    <Label x={2} y={25.3} width={44} size={0.57} color={muted}>{"Width is chosen first; then text wraps. A row does not resize fixed siblings to the final text height."}</Label>
  </Group>
</Slide>
