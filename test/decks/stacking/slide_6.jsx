// Stacking scenarios / 06. Read README.md for the deck and render commands.
...
return <Slide background={paper} border={0} margin={0} padding={0}
  ...
  <Group coord={[0, 0, 48, 27]} aspect={16 / 9}>
    <Label x={2} y={1.25} width={44} size={0.55} color={teal} family={mono}>
      STACKING / 06 OF 07 / UNIFIED METRICS
    </Label>
    <Label x={2} y={2.55} width={44} size={1.4} weight={bold}>Choose the width, keep the em.</Label>
    <Label x={2} y={4.55} width={44} size={0.76} color={muted}>Natural and explicit dimensions come first. Growth receives the remaining space.</Label>
    <Label x={2} y={6} width={44} size={0.5} color={muted} family={mono}>
      Dashed outlines = allocated boxes. Numbers = width × height.
    </Label>
    <Panel x={2} title="grow: equal child weights" stack={first} unit={1.35}
    ...
    <Panel x={24.5} title="width + grow" stack={second} unit={1.35}
      code={"<HStack width={12} height={3} gap={1}>\n  <Text width={2}>Hi</Text>\n  <Text grow={1}>Long label</Text>\n</HStack>"}
      note="The first child takes 2 em. After the 1-em gap, the second gets 9 em. Both keep the same font size." />
    <Label x={2} y={25.3} width={44} size={0.57} color={muted}>A declared width uses layout units. grow divides what remains after natural children and gaps.</Label>
  </Group>
</Slide>
