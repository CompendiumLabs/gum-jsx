// Stacking scenarios / 02. Read README.md for the deck and render commands.
...
return <Slide background={paper} border={0} margin={0} padding={0}
  ...
  <Group coord={[0, 0, 48, 27]} aspect={16 / 9}>
    <Label x={2} y={1.25} width={44} size={0.55} color={teal} family={mono}>
      STACKING / 02 OF 07 / UNIFIED METRICS
    </Label>
    <Label x={2} y={2.55} width={44} size={1.4} weight={bold}>Two shapes, equal or unequal aspects.</Label>
    <Label x={2} y={4.55} width={44} size={0.76} color={muted}>With a width budget, aspect-only siblings solve for a common height.</Label>
    <Label x={2} y={6} width={44} size={0.5} color={muted} family={mono}>
      Dashed outlines = allocated boxes. Numbers = width × height.
    </Label>
    <Panel x={2} title="Matching aspects: 2 and 2" stack={first} unit={1.1}
      code={"<HStack width={12} gap={1}>\n  <Rect aspect={2} />\n  <Rect aspect={2} />\n</HStack>"}
      note="After the gap, 11 units remain. Common height = 11 / (2 + 2) = 2.75. Each shape gets width 5.5." />
    <Panel x={24.5} title="Different aspects: 2 and 0.5" stack={second} unit={1.1}
      code={"<HStack width={12} gap={1}>\n  <Rect aspect={2} />\n  <Rect aspect={0.5} />\n</HStack>"}
      note="Common height = 11 / (2 + 0.5) = 4.4. Widths become 8.8 and 2.2. Different aspects still share a height." />
    <Label x={2} y={25.3} width={44} size={0.57} color={muted}>{"Both panels use the same diagram scale. Width = aspect × common height."}</Label>
  </Group>
</Slide>
