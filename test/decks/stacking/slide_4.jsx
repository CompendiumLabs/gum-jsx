// Stacking scenarios / 04. Read README.md for the deck and render commands.
...
return <Slide background={paper} border={0} margin={0} padding={0}
  ...
  <Group coord={[0, 0, 48, 27]} aspect={16 / 9}>
    <Label x={2} y={1.25} width={44} size={0.55} color={teal} family={mono}>
      STACKING / 04 OF 07 / UNIFIED METRICS
    </Label>
    <Label x={2} y={2.55} width={44} size={1.4} weight={bold}>Match the shape to its slot.</Label>
    <Label x={2} y={4.55} width={44} size={0.76} color={muted}>The reserved rectangle and the visible drawing have separate dimensions.</Label>
    <Label x={2} y={6} width={44} size={0.5} color={muted} family={mono}>
      Dashed outlines = allocated boxes. Numbers = width × height.
    </Label>
    <Panel x={2} title="6 × 3 slot, 2:1 drawing" stack={first} unit={1.35}
    ...
    <Panel x={24.5} title="6 × 3 slot, 1:1 drawing" stack={second} unit={1.35}
      code={"<HStack gap={1}>\n  <Rect width={6} height={3}\n    aspect={1} />\n  <Text width={4}>Hi</Text>\n</HStack>"}
      note="The square draws at 3 × 3 inside the same 6 × 3 slot. The empty room remains reserved; the label does not move." />
    <Label x={2} y={25.3} width={44} size={0.57} color={muted}>{"Dashed child outlines show allocations. Filled geometry shows what actually draws."}</Label>
  </Group>
</Slide>
