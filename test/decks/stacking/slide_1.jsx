// Stacking scenarios / 01. Read README.md for the deck and render commands.
...
return <Slide background={paper} border={0} margin={0} padding={0}
  ...
  <Group coord={[0, 0, 48, 27]} aspect={16 / 9}>
    <Label x={2} y={1.25} width={44} size={0.55} color={teal} family={mono}>
      STACKING / 01 OF 07 / UNIFIED METRICS
    </Label>
    <Label x={2} y={2.55} width={44} size={1.4} weight={bold}>A shape meets a text label.</Label>
    <Label x={2} y={4.55} width={44} size={0.76} color={muted}>Aspect describes a shape. Natural dimensions give it a size.</Label>
    <Label x={2} y={6} width={44} size={0.5} color={muted} family={mono}>
      Dashed outlines = allocated boxes. Numbers = width × height.
    </Label>
    <Panel x={2} title="Shared height, inferred" stack={first} unit={1.35}
      code={"<HStack gap={1}>\n  <Rect aspect={2} />\n  <Text>Hi</Text>\n</HStack>"}
      note="The label is one em tall. The unsized 2:1 shape follows that height and becomes 2 × 1." />
    <Panel x={24.5} title="Different heights, explicit" stack={second} unit={1.35}
    ...
    <Label x={2} y={25.3} width={44} size={0.57} color={muted}>{"Matching heights can be inferred. An explicit child size takes precedence."}</Label>
  </Group>
</Slide>
