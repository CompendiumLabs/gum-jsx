...
- `test/decks/` - Slide decks, one directory of `slide_N.jsx` files each; rendered by the suite like the examples and browsed in the report one deck at a time
...
`--report` through — and every deck under `test/decks` (each subdirectory, its slides in
numeric order). Every example and slide renders in **strict mode**
